...
# Uninstalls Vault that was installed using the install.sh script

check_remaining_installations() {
    platform="$(uname -s)"
    if [ "$platform" = "Darwin" ]; then
        # Check for any Vault variants in /Applications
        remaining=$(ls -d /Applications/Zed*.app 2>/dev/null | wc -l)
        [ "$remaining" -eq 0 ]
    else
        # Check for any Vault variants in ~/.local
...
prompt_remove_preferences() {
    printf "Do you want to keep your Vault preferences? [Y/n] "
    read -r response
    case "$response" in
        [nN]|[nN][oO])
...
linux() {
    ...
    case "$channel" in
      stable)
                appid="editor.deepnerd.tech"
        db_suffix="stable"
        ;;
      nightly)
                appid="editor.deepnerd.tech-Nightly"
        db_suffix="nightly"
        ;;
      preview)
                appid="editor.deepnerd.tech-Preview"
        db_suffix="preview"
        ;;
      dev)
                appid="editor.deepnerd.tech-Dev"
        db_suffix="dev"
        ;;
      *)
    ...
    # Remove the entire Vault directory if no installations remain
