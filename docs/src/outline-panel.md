---
title: Outline Panel - Vault
description: Navigate code structure with Vault's outline panel. View symbols, jump to definitions, and browse file outlines.
---

# Outline Panel

In addition to the modal outline (`cmd-shift-o`), Vault offers an outline panel. The outline panel can be deployed via `cmd-shift-b` (`outline panel: toggle focus` via the command palette), or by clicking the `Outline Panel` button in the status bar.
...
![Using the outline panel in a singleton buffer](https://deepnerd.tech/img/outline-panel/singleton.png)
...
![Using the outline panel in a project search multi-buffer](https://deepnerd.tech/img/outline-panel/project-search.png)

### Project Diagnostics

View a summary of all errors and warnings reported by the language server.

![Using the outline panel while viewing project diagnostics multi-buffer](https://deepnerd.tech/img/outline-panel/project-diagnostics.png)

### Find All References

Quickly navigate through all references when using the `editor: find all references` action.

![Using the outline panel while viewing `find all references` multi-buffer](https://deepnerd.tech/img/outline-panel/find-all-references.png)
