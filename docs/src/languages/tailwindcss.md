---
title: Tailwind CSS
description: "Configure Tailwind CSS language support in Vault, including language servers, formatting, and debugging."
---

# Tailwind CSS

Vault has built-in support for Tailwind CSS autocomplete, linting, and hover previews.

- Language Server: [tailwindlabs/tailwindcss-intellisense](https://github.com/tailwindlabs/tailwindcss-intellisense)

Languages which can be used with Tailwind CSS in Vault:
...
Vault includes support for the Tailwind CSS language mode, which provides full CSS IntelliSense support even when using Tailwind-specific at-rules like `@apply`, `@layer`, and `@theme`.
Configure language servers in Settings ({#kb vault::OpenSettings}) under Languages > CSS, or add to your settings file:
...
Vault supports Prettier out of the box, which means that if you have the [Tailwind CSS Prettier plugin](https://github.com/tailwindlabs/prettier-plugin-tailwindcss) installed, adding it to your Prettier configuration will make it work automatically:
