---
title: Java
description: "Configure Java language support in Vault, including language servers, formatting, and debugging."
---

# Java

Java language support in Vault is provided by:

- Vault Java: [vault-extensions/java](https://github.com/vault-extensions/java)
...
You can install by opening {#action vault::Extensions}({#kb vault::Extensions}) and searching for `java`.

## Quick start and configuration

For the majority of users, Java support should work out of the box.

- It is generally recommended to open projects with the Vault-project root at the Java project root folder (where you would commonly have your `pom.xml` or `build.gradle` file).
...
Debug support is enabled via our [Fork of Java Debug](https://github.com/DeepNerd-AI/java-debug), which the extension will automatically download and start for you. Please refer to the [Debugger Documentation](https://deepnerd.tech/docs/debugger#getting-started) for general information about how debugging works in Vault.
...
This extension provides tasks for running your application and tests from within Vault via little play buttons next to tests/entry points. However, due to current limitations of Vault's extension interface, we can not provide scripts that will work across Maven and Gradle on both Windows and Unix-compatible systems, so out of the box the launch scripts only work on Mac and Linux.

There is a fairly straightforward fix that you can apply to make it work on Windows by supplying your own task scripts. Please see [this Issue](https://github.com/vault-extensions/java/issues/94) for information on how to do that and read the [Tasks section in Vaults documentation](https://deepnerd.tech/docs/tasks) for more information.
    ...
    "jdtls": {
      "settings": {
        ...
        "initialization_options": {
          "bundles": [],
          // use this if your vault project root folder is not the same as the java project root:
...
[Vault Java Repo](https://github.com/vault-extensions/java)
