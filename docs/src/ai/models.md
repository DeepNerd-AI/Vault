---
title: AI Models and Pricing - Vault
description: AI models available via Vault Pro including Claude, GPT-5.4, Gemini 3.1 Pro, and Grok. Pricing, context windows, and tool call support.
---

# Models

Vault's plans offer hosted versions of major LLMs with higher rate limits than direct API access. Model availability is updated regularly. To use your own API keys instead, see [LLM Providers](./llm-providers.md). For general setup, see [Configuration](./configuration.md).
...
As of February 19, 2026, Vault Pro serves newer model versions in place of the retired models below:
...
Any usage of a Vault-hosted model will be billed at the Vault Price (rightmost column above). See [Plans and Usage](./plans-and-usage.md) for details on Vault's plans and limits for use of hosted models.
...
Each Agent thread in Vault maintains its own context window.
