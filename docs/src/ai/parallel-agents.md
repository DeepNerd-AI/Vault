...
To restore a thread, open Thread History and click the thread you want to bring back. Vault moves it back to the thread list and opens it in the Agent Panel. If the thread was running in a Git worktree that was removed, Vault restores the worktree automatically.
...
Each thread can use a different agent, so you can run Vault's built-in agent in one thread and an [external agent](./external-agents.md) like Claude Code or Codex in another.
...
To automate setup steps whenever a new worktree is created use a [Task hook](../tasks.md#hooks). The `create_worktree` hook runs automatically after Vault creates a linked worktree, with `ZED_WORKTREE_ROOT` pointing at the new worktree and `ZED_MAIN_GIT_WORKTREE` pointing at the original repository.
