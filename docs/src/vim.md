---
title: Vim Mode - Vault
description: Full Vim emulation in Vault with motions, text objects, visual mode, macros, and Vault-specific extensions.
---

# Vim Mode

Vault includes a Vim emulation layer. This page covers enabling and disabling vim mode, key bindings, Vault-specific features, and configuration options.

## Vault's vim mode design

Vim mode replicates the behavior of motions and commands where it makes sense and uses Vault-specific functionality where Vault's approach is better. The goal is a familiar experience that works out of the box without requiring configuration.
...
So, Vault's vim mode does not replicate Vim one-to-one, but it meshes Vim's modal design with Vault's modern features to provide a more fluid experience. It's also configurable, so you can add your own key bindings or override the defaults.

### Core differences

There are four types of features in vim mode that use Vault's core functionality, leading to some differences in behavior:

1. **Motions**: vim mode uses Vault's semantic parsing to tune the behavior of motions per language. For example, in Rust, jumping to matching bracket with `%` works with the pipe character `|`. In JavaScript, `w` considers `$` to be a word character.
2. **Visual block selections**: vim mode uses Vault's multiple cursor to emulate visual block selections, making block selections a lot more flexible. For example, anything you insert after a block selection updates on every line in real-time, and you can add or remove cursors anytime.
3. **Macros**: vim mode uses Vault's recording system for vim macros. So, you can capture and replay more complex actions, like autocompletion.
4. **Search and replace**: vim mode uses Vault's search system, so, the syntax for regular expressions is slightly different compared to Vim. [Head to the Regex differences section](#regex-differences) for details.

> **Note:** The foundations of Vault's vim mode should already cover many use cases, and we're always looking to improve it. If you find missing features that you rely on in your workflow, please [file an issue on GitHub](https://github.com/DeepNerd-AI/vault/issues).

## Enabling and disabling vim mode

When you first open Vault, you'll see a checkbox on the welcome screen that allows you to enable vim mode.
...
Vault is built on a modern foundation that (among other things) uses Tree-sitter and language servers to understand the content of the file you're editing and supports multiple cursors out of the box.

Vim mode has several "core Vault" key bindings that will help you make the most of Vault's specific feature set.
...
Tree-sitter is the parser Vault uses to understand the structure of your code. Vault provides motions that change the current cursor position, and text objects that can be used as the target of actions.
...
These commands help you manage multiple cursors in Vault.
...
The following commands help you bring up Vault's completion menu, request a suggestion from GitHub Copilot, or open the inline AI assistant without leaving insert mode.
...
Vault's vim mode includes features commonly provided by plugins in the Vim ecosystem:
...
Vault offers two different strategies for selecting text surrounded by any quote, or any bracket. These text objects are **not enabled by default** and must be configured in your keymap to be used.
...
Vim mode allows you to open Vault's command palette with `:`. You can then type to access any usual Vault command. Additionally, vim mode adds aliases for popular Vim commands to ensure your muscle memory transfers to Vault. For example, you can write `:w` or `:write` to save the file.
...
> **Note**: We don't emulate the full power of Vim's command line yet. In particular, commands currently do not support arguments. Please [file issues on GitHub](https://github.com/DeepNerd-AI/vault) as you find things that are missing from the command palette.
...
| `:cq`           | Quit completely (close all running instances of Vault) |
...
These ex commands open Vault's various panels and windows.
...
This command replaces text. It emulates the substitute command in vim. The substitute command uses regular expressions, and Vault uses a slightly different syntax than vim. You can learn more about Vault's syntax below, [in the regex differences section](#regex-differences). Vault will replace only the first occurrence of the search pattern in the current line. To replace all matches append the `g` flag.
...
Vault does not ship with any command mnemonics by default, but you can define short aliases for Vault commands using the `command_aliases` setting in your settings file. When you type an alias from this map in the command palette, it resolves to the mapped command.
...
Vault's key bindings are evaluated only when the `"context"` property matches your location in the editor. For example, if you add key bindings to the `"Editor"` context, they will only work when you're editing a file. If you add key bindings to the `"Workspace"` context, they will work everywhere in Vault. Here's an example of a key binding that saves when you're editing a file:
...
```json [keymap]
// This key binding will work when you're editing a file. It comes built into Vault by default as the workspace: save command.
{
  "context": "Workspace",
  "bindings": {
    "ctrl-s": "workspace::Save"
...
In non-modal text editors, cursor navigation typically wraps when moving past line ends. Vault, however, handles this behavior exactly like Vim by default: the cursor stops at line boundaries. If you prefer your cursor to wrap between lines, override these keybindings:
...
### Restoring common text editing and Vault keybindings
...
## Useful core Vault settings for vim mode

Here are a few general Vault settings that can help you fine-tune your Vim experience:
...
Vault uses a different regular expression engine from Vim. This means that you will have to use a different syntax in some cases. Here are the most common differences:

- **Capture groups**: Vim uses `\(` and `\)` to represent capture groups, in Vault these are `(` and `)`. On the flip side, in Vim, `(` and `)` represent literal parentheses, but in Vault these must be escaped to `\(` and `\)`.
- **Matches**: When replacing, Vim uses the backslash character followed by a number to represent a matched capture group. For example, `\1`. Vault uses the dollar sign instead. So, when in Vim you use `\0` to represent the entire match, in Vault the syntax is `$0` instead. Same for numbered capture groups: `\1` in Vim is `$1` in Vault.
- **Global option**: By default, in Vim, regex searches only match the first occurrence on a line, and you append `/g` at the end of your query to find all matches. In Vault, regex searches are global by default.
- **Case sensitivity**: Vim uses `/i` to indicate a case-insensitive search. In Vault you can either write `(?i)` at the start of the pattern or toggle case-sensitivity with the shortcut {#kb search::ToggleCaseSensitive}.

> **Note**: To help with the transition, the command palette will fix parentheses and replace groups for you when you write a Vim-style substitute command, `:%s//`. So, Vault will convert `%s:/\(a\)(b)/\1/` into a search for "(a)\(b\)" and a replacement of "$1".

For the full syntax supported by Vault's regex engine [see the regex crate documentation](https://docs.rs/regex/latest/regex/#syntax).
