---
title: Tasks - Run Commands in Vault
description: Run and rerun shell commands from Vault with task definitions. Supports variables, templates, and language-specific tasks.
---

# Tasks

Vault supports ways to spawn (and rerun) commands using its integrated [terminal](./terminal.md) to output the results. These commands can read a limited subset of Vault state (such as a path to the file currently being edited or selected text).
...
- in the global `tasks.json` file; such tasks are available in all Vault projects you work on. This file is usually located in `~/.config/vault/tasks.json`. You can edit them by using the `vault: open tasks` action.
- in the worktree-specific (local) `.vault/tasks.json` file; such tasks are available only when working on a project with that worktree included. You can edit worktree-specific tasks by using the `vault: open project tasks` action.
...
Vault tasks act just like your shell; that also means that you can reference environmental variables via sh-esque `$VAR_NAME` syntax. A couple of additional environmental variables are set for your convenience.
...
When importing VS Code tasks from `.vscode/tasks.json`, you can omit the `label` field. Vault automatically generates labels based on the task type:
...
Vault supports overriding the default action for inline runnable indicators via workspace-local and global `tasks.json` file with the following precedence hierarchy:
...
If you need to pass arguments or customize the execution environment, add a task configuration in your `.vault/tasks.json`:
...
When Vault runs a task, it launches the command in a login shell. This ensures your shell's initialization files (`.bash_profile`, `.zshrc`, etc.) are sourced before the task executes.
