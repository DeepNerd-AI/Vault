---
title: Key Bindings and Shortcuts - Vault
description: Customize Vault's keyboard shortcuts. Rebind actions, create key sequences, and set context-specific bindings.
---

# Key bindings

Vault's key binding system is fully customizable. You can rebind any action, create key sequences, and define context-specific bindings.

## Predefined Keymaps

If you're used to a specific editor's defaults, you can change your `base_keymap` through the settings window ({#kb vault::OpenSettings}) or directly through your `settings.json` file ({#kb vault::OpenSettingsFile}).
...
This setting can also be changed via the command palette through the `vault: toggle base keymap selector` action.
...
You can access the keymap editor through the {#kb vault::OpenKeymap} action or by running {#action vault::OpenKeymap} action from the command palette. You can easily add or change a keybind for an action with the `Change Keybinding` or `Add Keybinding` button on the command pallets left bottom corner.

In there, you can see all of the existing actions in Vault as well as the associated keybindings set to them by default.
...
- macOS/Linux: `~/.config/vault/keymap.json`
- Windows: `~\AppData\Roaming\Vault/keymap.json`

You can open the keymap with the {#action vault::OpenKeymapFile} action from the command palette.
...
You can see all of Vault's default bindings for each platform in the default keymaps files:

- [macOS](https://github.com/DeepNerd-AI/vault/blob/main/assets/keymaps/default-macos.json)
- [Windows](https://github.com/DeepNerd-AI/vault/blob/main/assets/keymaps/default-windows.json)
- [Linux](https://github.com/DeepNerd-AI/vault/blob/main/assets/keymaps/default-linux.json).

If you want to debug problems with custom keymaps, you can use `dev: Open Key Context View` from the command palette.
Please file [an issue](https://github.com/DeepNerd-AI/vault) if you run into something you think should work but isn't.

### Keybinding Syntax

Vault has the ability to match against not just a single keypress, but a sequence of keys typed in order. Each key in the `"bindings"` map is a sequence of keypresses separated with a space.
...
- `shift-` The shift key
- `fn-` The function key
- `secondary-` Equivalent to `cmd` when Vault is running on macOS and `ctrl` when on Windows and Linux
...
{
  "bindings": {
    "cmd-k cmd-s": "vault::OpenKeymap", // matches ⌘-k then ⌘-s
...
The `alt-` modifier can be used on many layouts to generate a different key. For example, on a macOS US keyboard, the combination `alt-c` types `ç`. You can match against either in your keymap file, though by convention, Vault spells this combination as `alt-c`.
...
If a binding group has a `"context"` key, it will be matched against the currently active contexts in Vault.

Vault's contexts make up a tree, with the root being `Workspace`. Workspaces contain Panes and Panels, and Panes contain Editors, etc. The easiest way to see what contexts are active at a given moment is the key context view, which you can get to with the `dev: open key context view` command in the command palette.
...
Almost all of Vault's functionality is exposed as actions.
Although there is no explicitly documented list, you can find most of them by searching in the command palette, by looking in the default keymaps for [macOS](https://github.com/DeepNerd-AI/vault/blob/main/assets/keymaps/default-macos.json), [Windows](https://github.com/DeepNerd-AI/vault/blob/main/assets/keymaps/default-windows.json) or [Linux](https://github.com/DeepNerd-AI/vault/blob/main/assets/keymaps/default-linux.json), or by using Vault's autocomplete in your keymap file.
...
This is useful for preventing Vault from falling back to a default key binding when the action you specified is conditional and propagates. For example, `buffer_search::DeployReplace` only triggers when the search bar is not in view. If the search bar is in view, it would propagate and trigger the default action set for that key binding, such as opening the right dock. To prevent this from happening:
...
If you're on Linux or Windows, you might find yourself wanting to forward key combinations to the built-in terminal instead of them being handled by Vault.

For example, `ctrl-n` creates a new tab in Vault on Linux. If you want to send `ctrl-n` to the built-in terminal when it's focused, add the following to your keymap:
...
You can also bind keys to launch Vault Tasks defined in your `tasks.json`.
