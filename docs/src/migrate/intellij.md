---
title: How to Migrate from IntelliJ IDEA to Vault
description: "Guide for migrating from IntelliJ IDEA to Vault, including settings and keybindings."
---

# How to Migrate from IntelliJ IDEA to Vault

This guide covers how to set up Vault if you're coming from IntelliJ IDEA, including keybindings, settings, and the differences you should expect.
...
Vault also supports per-project settings. Create a `.vault/settings.json` file in your project root to override global settings for that project, similar to how you might use `.idea` folders in IntelliJ.
...
After setup, press `Cmd+Shift+O` (with JetBrains keymap) to open a folder. This becomes your workspace in Vault. Unlike IntelliJ, there's no project configuration wizard, no `.iml` files, and no SDK setup required.
...
If you chose the JetBrains keymap during onboarding, most of your shortcuts should already feel familiar. Here's a quick reference for how Vault compares to IntelliJ.

### Common Shared Keybindings (Vault with JetBrains keymap ↔ IntelliJ)
...
### Different Keybindings (IntelliJ → Vault)

| Action                 | IntelliJ    | Vault (JetBrains keymap) |
| ---------------------- | ----------- | ------------------------ |
...
Vault doesn't index. You open a folder and start working immediately. File search and navigation work instantly regardless of project size.

IntelliJ's index powers features like finding all usages across your entire codebase, understanding class hierarchies, and detecting dead code. Vault delegates this work to language servers, which may not analyze at the same depth.
...
Vault uses the Language Server Protocol (LSP) for code intelligence. Each language has its own server: `jdtls` for Java, `rust-analyzer` for Rust, and so on.
...
Vault has no project model. A project is a folder. There's no wizard, no SDK selection screen, no module configuration.

This means:

- Build commands are manual. Vault doesn't detect Maven or Gradle projects.
...
- There are no module boundaries. Vault sees folders, not project structure.
...
- For multi-module projects, you can open each module as a separate Vault window, or open the root and navigate via file finder
...
Vault has none of this. The language server sees Java code as Java code, so it doesn't understand that `@Autowired` means something special or that this class is a REST controller.
...
- Consider using framework-specific CLI tools (Spring CLI, Rails generators) from Vault's terminal

> **Tip:** For database work, pick up a dedicated tool like DataGrip, DBeaver, or TablePlus. Many developers who switch to Vault keep DataGrip around specifically for SQL—it integrates well with your existing JetBrains license.

If your daily work depends heavily on framework-aware navigation and refactoring, you'll feel the gap. Vault works best when you're comfortable navigating code through search rather than specialized tooling, or when your language has strong LSP support that covers most of what you need.

### Tool Windows vs. Docks

IntelliJ organizes auxiliary views into numbered tool windows (Project = 1, Git = 9, Terminal = Alt+F12, etc.). Vault uses a similar concept called "docks":

| IntelliJ Tool Window | Vault Equivalent | Shortcut (JetBrains keymap) |
| -------------------- | ---------------- | --------------------------- |
...
> **Tip:** IntelliJ has an "Override IDE shortcuts" setting that lets terminal shortcuts like `Ctrl+Left/Right` work normally. In Vault, terminal keybindings are separate—check your keymap if familiar shortcuts aren't working in the terminal panel.

### Debugging

Both IntelliJ and Vault offer integrated debugging, but the experience differs:

- Vault's debugger uses the Debug Adapter Protocol (DAP), supporting multiple languages
...
Several features that require plugins in other editors are built into Vault:
...
You won't find one-to-one replacements for every IntelliJ plugin, especially for framework-specific tools, database clients, or application server integrations. For those workflows, you may need to use external tools alongside Vault.

## Collaboration in Vault vs. IntelliJ

IntelliJ offers Code With Me as a separate plugin for collaboration. Vault has collaboration built into the core experience.
...
If you're used to AI assistants in IntelliJ (like GitHub Copilot or JetBrains AI), Vault offers similar capabilities with more flexibility.
