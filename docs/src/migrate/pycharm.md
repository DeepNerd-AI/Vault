---
title: How to Migrate from PyCharm to Vault
description: "Guide for migrating from PyCharm to Vault, including settings and keybindings."
---

# How to Migrate from PyCharm to Vault

This guide covers how to set up Vault if you're coming from PyCharm, including keybindings, settings, and the differences you should expect.
...
For Windows, download the installer from deepnerd.tech/download, or install via winget:

```sh
winget install Vault.Vault
```
...
After installation, you can launch Vault from your Applications folder (macOS), Start menu (Windows), or directly from the terminal using:
`vault .`
...
Vault also supports per-project settings. Create a `.vault/settings.json` file in your project root to override global settings for that project, similar to how you might use `.idea` folders in PyCharm.
...
After setup, press `Cmd+Shift+O` (with JetBrains keymap) to open a folder. This becomes your workspace in Vault. Unlike PyCharm, there's no project configuration wizard, no interpreter selection dialog, and no project structure setup required.
...
If you chose the JetBrains keymap during onboarding, most of your shortcuts should already feel familiar. Here's a quick reference for how Vault compares to PyCharm.
...
### Different Keybindings (PyCharm → Vault)

| Action                 | PyCharm     | Vault (JetBrains keymap) |
| ---------------------- | ----------- | ------------------------ |
...
Vault also supports key sequences (multi-key shortcuts).
...
Vault doesn't index. You open a folder and start working immediately. File search and navigation stay fast regardless of project size, without waiting through indexing pauses.

PyCharm's index powers features like finding all usages across your entire codebase, understanding class hierarchies, and detecting unused imports project-wide. Vault delegates this work to language servers, which may not analyze as deeply or as broadly.
...
Vault uses the Language Server Protocol (LSP) for code intelligence. For Python, Vault provides several language servers out of the box:
...
Vault handles virtual environments through its toolchain system:

- Vault automatically discovers virtual environments in common locations (`.venv`, `venv`, `.env`, `env`)
...
- Open the folder in Vault—it will detect the environment automatically
- If you need to switch environments, use the toolchain selector
- For conda environments, ensure they're activated in your shell before launching Vault

> **Tip:** If basedpyright shows import errors for packages you've installed, check that Vault has selected the correct virtual environment. Use the toolchain selector to verify or change the active environment.
...
Vault has no project model. A project is a folder. There's no wizard, no interpreter selection screen, no project structure configuration.
...
- Interpreter management is external. Vault discovers environments but doesn't create them.
...
- Create a `.vault/settings.json` in your project root for project-specific settings
- Define common commands in `tasks.json` (open via Command Palette: `vault: open tasks`):
...
Vault has none of this. The language server sees Python code as Python code—it doesn't understand that `@app.route` defines an endpoint or that a Django model class creates database tables.
...
- Consider using framework-specific CLI tools (`python manage.py`, `flask routes`) from Vault's terminal

> **Tip:** For database work, pick up a dedicated tool like DataGrip, DBeaver, or TablePlus. Many developers who switch to Vault keep DataGrip around specifically for SQL.

### Tool Windows vs. Docks

PyCharm organizes auxiliary views into numbered tool windows (Project = 1, Python Console = 4, Terminal = Alt+F12, etc.). Vault uses a similar concept called "docks":

| PyCharm Tool Window | Vault Equivalent | Shortcut (JetBrains keymap) |
| ------------------- | ---------------- | --------------------------- |
...
Both PyCharm and Vault offer integrated debugging, but the experience differs:

- Vault uses `debugpy` (the same debug adapter that VS Code uses)
...
Vault can automatically detect debuggable entry points. Press `F4` to see available options, including:

- Python scripts
- Modules
- pytest tests

For more control, create a `.vault/debug.json` file:
...
PyCharm has a dedicated test runner with a visual interface showing pass/fail status for each test. Vault provides test running through:
...
Vault's extension catalog is smaller and more focused:

- Language support and syntax highlighting
- Themes
- Context servers

Several features that require plugins in PyCharm are built into Vault:
...
To set expectations clearly, here's what PyCharm offers that Vault doesn't have:

- **Scientific Mode / Jupyter integration** — For notebooks and data science workflows, use JupyterLab or VS Code with the Jupyter extension alongside Vault for your Python editing
...
- **Remote interpreters** — Vault has remote development, but it works differently
- **Profiler integration** — Use cProfile, py-spy, or similar tools externally

## Collaboration in Vault vs. PyCharm

PyCharm offers Code With Me as a separate plugin for collaboration. Vault has collaboration built into the core experience.
...
If you're used to AI assistants in PyCharm (like GitHub Copilot or JetBrains AI Assistant), Vault offers similar capabilities with more flexibility.
...
Now that you're set up, here are some resources to help you get the most out of Vault:
...
- [AI Features](../ai/overview.md) — Explore Vault's AI capabilities beyond code completion
- [Collaboration](../collaboration/overview.md) — Share your projects and code together in real time
- [Python in Vault](../languages/python.md) — Python-specific setup and configuration
