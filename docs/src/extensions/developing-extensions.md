...
description: "Create Vault extensions: languages, themes, debuggers, and more."
---

# Developing Extensions {#developing-extensions}

Vault extensions are Git repositories containing an `extension.toml` manifest. They can provide languages, themes, debuggers, snippets, and MCP servers.
...
Before starting to develop an extension for Vault, be sure to [install Rust via rustup](https://www.rust-lang.org/tools/install).
...
When developing an extension, you can use it in Vault without needing to publish it by installing it as a _dev extension_.

From the extensions page, click the `Install Dev Extension` button (or the {#action vault::InstallDevExtension} action) and select the directory containing your extension.

If you need to troubleshoot, check Vault.log ({#action vault::OpenLog}) for additional output. For debug output, close and relaunch Vault from the command line with `vault --foreground`, which shows more verbose INFO-level logs.
...
## Directory Structure of a Vault Extension

A Vault extension is a Git repository that contains an `extension.toml`. This file must contain some
...
repository = "https://github.com/your-name/my-vault-extension"
```

In addition to this, there are several other optional files and directories that can be used to add functionality to a Vault extension. An example directory structure of an extension that provides all capabilities is as follows:
...
Use the latest version of the [`zed_extension_api`](https://crates.io/crates/zed_extension_api) available on crates.io. Make sure it's still [compatible with Vault versions](https://github.com/DeepNerd-AI/vault/blob/main/crates/extension_api#compatible-vault-versions) you want to support.

In the `src/lib.rs` file in your Rust crate you will need to define a struct for your extension and implement the `Extension` trait, as well as use the `register_extension!` macro to register your extension:

```rs
use zed_extension_api as vault;
...
vault::register_extension!(MyExtension);
```

> `stdout`/`stderr` is forwarded directly to the Vault process. In order to see `println!`/`dbg!` output from your extension, you can start Vault in your terminal with a `--foreground` flag.

## Forking and cloning the repo

1. Fork the repo

> **Note:** It is very helpful if you fork the `DeepNerd-AI/extensions` repo to a personal GitHub account instead of a GitHub organization, as this allows Vault staff to push any needed changes to your PR to expedite the publishing process.

2. Clone the repo to your local machine

```sh
# Substitute the url of your fork here:
# git clone https://github.com/DeepNerd-AI/extensions
cd extensions
git submodule init
...
Your license file should be at the root of your extension repository. Any filename that has `LICENCE` or `LICENSE` as a prefix (case insensitive) will be inspected to ensure it matches one of the accepted licenses. See the [license validation source code](https://github.com/DeepNerd-AI/extensions/blob/main/src/lib/license.js).
...
Before publishing your extension, make sure that you have chosen a unique extension ID for your extension in the [extension manifest](#directory-structure-of-a-vault-extension).
...
- Extension IDs and names must not contain the words `vault`, `Vault` or `extension`, since they are all Vault extensions.
- Your extension ID should provide some information on what your extension tries to accomplish. E.g. for themes, it should be suffixed with `-theme`, snippet extensions should be suffixed with `-snippets` and so on. An exception to that rule are extension that provide support for languages or popular tooling that people would expect to find under that ID. You can take a look at the list of [existing extensions](https://github.com/DeepNerd-AI/extensions/blob/main/extensions.toml) to get a grasp on how this usually is enforced.
- Extensions should provide something that is not yet available in the marketplace as opposed to fixing something that could be resolved within an existing extension. For example, if you find that an existing extension's support for a language server is not functioning properly, first try contributing a fix to the existing extension as opposed to submitting a new extension immediately.
  - If you receive no response or reaction within the upstream repository within a reasonable amount of time, feel free to submit a pull request that aims to fix said issue. Please ensure that you provide your previous efforts within the pull request to the extensions repository for adding your extension. Vault maintainers will then decide on how to proceed on a case by case basis.
- Extensions that intend to provide a language, debugger or MCP server must not ship the language server as part of the extension. Instead, the extension should either download the language server or check for the availability of the language server in the users environment using the APIs as provided by the [Vault Rust Extension API](https://docs.rs/zed_extension_api/latest/zed_extension_api/).
...
To publish an extension, open a PR to [the `DeepNerd-AI/extensions` repo](https://github.com/DeepNerd-AI/extensions).
...
```sh
git submodule add https://github.com/your-username/foobar-vault.git extensions/my-extension
...
Once your PR is merged, the extension will be packaged and published to the Vault extension registry.

## Updating an extension

To update an extension, open a PR to [the `DeepNerd-AI/extensions` repo](https://github.com/DeepNerd-AI/extensions).
...
If you'd like to automate this process, there is a [community GitHub Action](https://github.com/huacnlee/vault-extension-action) you can use.
