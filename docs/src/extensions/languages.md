...
Language support in Vault has several components:
...
Each language supported by Vault must be defined in a subdirectory inside the `languages` directory of your extension.
...
- `first_line_pattern` is a regular expression that can be used alongside `path_suffixes` (above) or `file_types` in settings to match files that should use this language. For example, Vault uses this to identify Shell Scripts by matching [shebang lines](https://github.com/DeepNerd-AI/vault/blob/main/crates/languages/src/bash/config.toml) in the first line of a script.
- `debuggers` is an array of strings that are used to identify debuggers in the language. When launching a debugger's `New Process Modal`, Vault will order available debuggers by the order of entries in this array.
...
Vault uses the syntax tree produced by the [Tree-sitter](https://tree-sitter.github.io) query language to implement
several features:
...
features in Vault, using [JSON syntax](https://www.json.org/json-en.html) as a guiding example.
...
Vault resolves them right-to-left: It first tries the rightmost capture, and if the current theme has no style for it, falls back to the next capture to the left, and so on.
...
Here Vault will first try to resolve `@variable` from the theme. If the theme defines a style for `@variable`, that style is used. Otherwise, Vault falls back to `@type`.
...
Vault uses these to highlight matching brackets: painting each bracket pair with a different color ("rainbow brackets") and highlighting the brackets if the cursor is inside the bracket pair.
...
The `textobjects.scm` file defines rules for navigating by text objects. This was added in Vault v0.165 and is currently used only in Vim mode.
...
For languages with closures, these typically should not count as functions in Vault. This is best-effort, however, because languages like JavaScript do not syntactically differentiate between closures and top-level function declarations.
...
Vault uses the [Language Server Protocol](https://microsoft.github.io/language-server-protocol/) to provide advanced language support.
...
This is useful when a language server reports custom (non-standard) semantic token types that aren't covered by Vault's built-in default rules. Extension-provided rules act as sensible defaults for that language — users can always override them via `semantic_token_rules` in their settings file, and built-in default rules are only used when neither user nor extension rules match.

#### Customizing Semantic Token Styles

Vault supports customizing the styles used for semantic tokens. You can define rules in your settings file, which customize how semantic tokens get mapped to styles in your theme.
...
3. **Default rules** — Vault's built-in rules for standard LSP token types.
...
If your language server supports additional languages, you can use `language_ids` to map Vault `languages` to the desired [LSP-specific `languageId`](https://microsoft.github.io/language-server-protocol/specifications/lsp/3.17/specification/#textDocumentItem) identifiers:
