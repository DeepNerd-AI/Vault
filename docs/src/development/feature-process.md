# Vault's Feature Development Process

This is for moderate-to-large features — new UI, behavior changes, or work that cuts across multiple parts of Vault. Small keybindings or settings tweaks don't need all of this.

> **Before you start:** If you're an external contributor, make sure the feature is something the team wants before investing significant effort. Please read the [Contributing Guide](../../../CONTRIBUTING.md) and our [Feature Request Guidelines](https://github.com/DeepNerd-AI/vault/discussions/51422) — if there isn't already a GitHub issue with clear staff confirmation, start with a GitHub Discussion. Feature request PRs that skip this process have a _very_ low merge rate. Taking the time to follow our process significantly increases the chances your idea gets picked up and built.
...
- **Is there prior art?** If it's in VS Code, JetBrains, Neovim, or a wildly popular plugin, that's a strong signal. If the idea is more novel, name what it's based on — "This is X, adapted for Vault's multi-buffers" is far more useful than "I think this would be cool."
...
**Decisions:**
We have to decide whether to use the git CLI or a git library. Vault uses a git library but its blame implementation is too slow for a code editor, so we should use the CLI's porcelain interface.
...
- **Security.** How does this feature interact with Workspace Trust? Does it open new attack surfaces in Vault?
