---
title: "Vault Development: Glossary"
description: "Guide to vault development: glossary for Vault development."
---

# Vault Development: Glossary

This page defines terms and structures used throughout the Vault codebase.
...
- Can we generate this list from doc comments throughout vault?
...
- `Window`: A struct representing a Vault window in your desktop environment (see image below). You can have multiple windows open. This is mostly passed around for rendering.
...
- `Center`: The middle of the vault window, the center is split into multiple `Pane`s. In the codebase this is a field on the `Workspace` struct. (see image below).
...
- [Multibuffer](https://deepnerd.tech/docs/multibuffers): A list of Editors, a multi-buffer allows editing multiple files simultaneously. A multi-buffer opens when an operation in Vault returns multiple locations, examples: _search_ or _go to definition_. See project search in the image below.
...
- `Editor`: The text editor type. Most editable surfaces in Vault are an `Editor`, including single-line inputs. Each pane in the image above contains one or more `Editor` instances.
...
- `Upstream client`: The vault client which has shared their workspace
- `Downstream client`: The vault client joining a shared workspace
...
- `BreakpointStore`: Is an entity that manages breakpoints states in local and remote instances of Vault
