...
fn register_actions(
    ...
    workspace
        ...
        .register_action(|_, _: &install_cli::RegisterVaultScheme, window, cx| {
            cx.spawn_in(window, async move |workspace, cx| {
                install_cli::register_zed_scheme(cx).await?;
                workspace.update_in(cx, |workspace, _, cx| {
                    struct RegisterVaultScheme;

                    workspace.show_toast(
                        Toast::new(
                            NotificationId::unique::<RegisterVaultScheme>(),
                            format!(
                                "vault:// links will now open in {}.",
            ...
            .detach_and_prompt_err(
                "Error registering vault:// scheme",
                window,
                cx,
                |_, _, _| None,
            );
