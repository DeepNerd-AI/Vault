...
use onboarding::{FIRST_OPEN, has_any_configured_ai_key, show_onboarding_view};
...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "Vault failed to launch";
...
fn fail_to_open_window(e: anyhow::Error, _cx: &mut App) {
    eprintln!(
        "Vault failed to open a window: {e:?}. See https://deepnerd.tech/docs/linux for troubleshooting steps."
    );
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    {
        ...
        _cx.spawn(async move |_cx| {
            ...
            let notification_id = "editor.deepnerd.tech.Oops";
            proxy
                .add_notification(
                    notification_id,
                    Notification::new("Vault failed to launch")
                        .body(Some(
                            format!(
                                "{e:?}. See https://deepnerd.tech/docs/linux for troubleshooting steps."
                            )
                            .as_str(),
                        ))
...
fn main() {
    ...
    if args.system_specs {
        ...
        println!("Vault System Specs (from CLI):\n{}", system_specs);
        return;
    }
...
pub(crate) async fn restore_or_create_workspace(
    ...
    if let Some(multi_workspaces) = restorable_workspaces(cx, &app_state).await {
        ...
        // check, Vault would exit silently.
...
struct Args {
    ...
    /// On macOS, the default is `~/Library/Application Support/Vault`.
    /// On Linux/FreeBSD, the default is `$XDG_DATA_HOME/zed`.
    /// On Windows, the default is `%LOCALAPPDATA%\Vault`.
    #[arg(long, value_name = "DIR", verbatim_doc_comment)]
    user_data_dir: Option<String>,

    /// The username and WSL distribution to use when opening paths. If not specified,
    /// Vault will attempt to open the paths directly.
    ///
    ...
    /// prevents Vault from starting, so you can't run `zed: copy system specs to
    /// clipboard`
    #[arg(long)]
    system_specs: bool,

    /// Used for the MCP Server, to remove the need for netcat as a dependency,
    /// by having Vault act like netcat communicating over a Unix socket.
    #[arg(long, hide = true)]
    nc: Option<String>,

    /// Used for recording minidumps on crashes by having Vault run a separate
    ...
    /// The PID of the Vault process to trace for heap analysis.
    #[cfg(target_os = "windows")]
    ...
    /// Unix socket path for IPC with the parent Vault process.
    #[cfg(target_os = "windows")]
    #[arg(long, hide = true)]
