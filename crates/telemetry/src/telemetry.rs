//! Telemetry layer (disabled). Events are no-op for Vault.
...
/// No-op telemetry event sender. Telemetry is disabled in Vault.
pub fn send_event(_event: Event) {
    // All telemetry is disabled for Vault. Events are discarded.
}

pub fn init(_tx: mpsc::UnboundedSender<Event>) {
    // Telemetry queue initialization is no-op. Connect to a queue is not performed.
}
