        ...
        let (error_kind, acp_error_code, message): (&str, Option<SharedString>, SharedString) =
            match error {
                ThreadError::PaymentRequired => (
                    "payment_required",
                    None,
                    "You reached your free usage limit. Upgrade to Vault Pro for more prompts."
                        .into(),
                ),
                ThreadError::Refusal => {
...
impl ThreadView {
    ...
    fn render_thread_controls(
        ...
        if enable_thread_feedback {
            ...
            let tooltip_meta = || {
                SharedString::new(
                    "Rating the thread sends all of your current conversation to the Vault team.",
                )
            };
    ...
    pub(crate) fn render_thread_error(
        ...
        let content = match self.thread_error.as_ref()? {
            ...
            ThreadError::RateLimitExceeded { provider } => self.render_error_callout(
                "Rate Limit Reached",
                format!(
                    "{provider}'s rate limit was reached. Vault will retry automatically. \
            ...
            ThreadError::ServerOverloaded { provider } => self.render_error_callout(
                ...
                format!(
                    "{provider}'s servers are temporarily unavailable. Vault will retry \
            ...
            ThreadError::StreamError { provider } => self.render_error_callout(
                ...
                format!(
                    "The connection to {provider}'s API was interrupted. Vault will retry \
            ...
            ThreadError::ApiError { provider } => self.render_error_callout(
                ...
                format!(
                    ...
                    If the problem persists, try switching models or restarting Vault."
                )
                .into(),
                true,
                true,
