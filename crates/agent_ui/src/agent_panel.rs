...
impl AgentPanel {
    ...
    fn active_initial_content(&self, cx: &App) -> Option<AgentInitialContent> {
        self.active_thread_view(cx).and_then(|thread_view| {
            ...
            if !entries.is_empty() {
                history_text.push_str("[System Note: The following is the conversation history transferred from the previous agent session]\n\n");
                for entry in entries {
                    history_text.push_str(&entry.to_markdown(cx));
            ...
            if !history_text.is_empty() {
                ...
                if let Some(draft) = draft_blocks {
                    blocks.extend(draft);
                ...
                // If there's no history, use the standard logic
                if let Some(draft) = draft_blocks {
                    if !draft.is_empty() {
                ...
                editor_text.map(|text| AgentInitialContent::ContentBlock {
            ...
            Rc::new(move |window, cx| {
                ...
                Some(ContextMenu::build(window, cx, |menu, _window, cx| {
                    menu.context(focus_handle.clone())
                        ...
                        .item(
                            ContextMenuEntry::new("DeepAgent")
