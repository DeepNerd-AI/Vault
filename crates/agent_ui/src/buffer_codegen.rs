...
impl CodegenAlternative {
    ...
    pub fn handle_stream(
        ...
        cx.spawn(async move |codegen, cx| {
            ...
            // If acquiring the stream failed (e.g., auth/connection), surface
            // the error on the codegen and stop gracefully instead of letting
            // the future propagate into panics or unexpected code paths.
            if let Err(err) = &stream {
                let err_str = format!("Failed to acquire model stream: {:#}", err);
                codegen
                    .update(cx, |this, cx| {
                        this.status = CodegenStatus::Error(anyhow::anyhow!(err_str.clone()));
                        this.elapsed_time = Some(start_time.elapsed().as_secs_f64());
