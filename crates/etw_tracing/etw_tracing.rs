...
fn start_etw_recording(cx: &mut App, heap_pid: Option<u32>) {
    ...
    let save_dialog = cx.prompt_for_new_path(&PathBuf::default(), Some("vault-trace.etl"));
...
<WindowsPerformanceRecorder Version="1.0" Author="DeepNerd-AI">
  <Profiles>
    {heap_provider}
...
pub fn launch_etw_recording(heap_pid: Option<u32>, output_path: &Path) -> Result<EtwSession> {
    let sock_path = std::env::temp_dir().join(format!("vault-etw-{}.sock", std::process::id()));
    ...
    let args = format!(
        "--record-etw-trace --etw-vault-pid {} --etw-output \"{}\" --etw-socket \"{}\"",
        pid_arg,
        output_path.display(),
        sock_path.display(),
    );
