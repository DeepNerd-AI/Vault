...
impl RenderOnce for ZedAiConfiguration {
    fn render(self, _window: &mut Window, _cx: &mut App) -> impl IntoElement {
        let (subscription_text, has_paid_plan) = match self.plan {
            Some(Plan::ZedPro) => (
                "You have access to Vault's hosted models through your Pro subscription.",
                true,
            ),
            Some(Plan::ZedProTrial) => (
                "You have access to Vault's hosted models through your Pro trial.",
                false,
            ),
            Some(Plan::ZedStudent) => (
                "You have access to Vault's hosted models through your Student subscription.",
                true,
            ),
            Some(Plan::ZedBusiness) => (
                if self.is_zed_model_provider_enabled {
                    "You have access to Vault's hosted models through your organization."
                } else {
                    "Vault's hosted models are disabled by your organization's configuration."
                },
                true,
            ),
            Some(Plan::ZedFree) | None => (
                if self.eligible_for_trial {
                    "Subscribe for access to Vault's hosted models. Start with a 14 day free trial."
                } else {
                    "Subscribe for access to Vault's hosted models."
                },
                false,
            ),
        };
        ...
        if !self.is_connected {
            return v_flex()
                .gap_2()
                .child(Label::new("Sign in to have access to Vault's complete agentic experience with hosted models."))
                .child(
                    Button::new("sign_in", "Sign In to use Vault AI")
