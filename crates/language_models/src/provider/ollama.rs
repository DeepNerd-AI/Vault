...
impl State {
    ...
    fn fetch_models(&mut self, cx: &mut Context<Self>) -> Task<Result<()>> {
        ...
        let native_api_url = ollama_native_api_url(&api_url);
        let api_key = if skip_auth_for_local_ollama(&api_url) {
            None
        } else {
            api_key
        };
        ...
        cx.spawn(async move |this, cx| {
            let models =
                get_models(http_client.as_ref(), &native_api_url, api_key.as_deref()).await?;
            ...
            let tasks = models
                ...
                .map(|model| {
                    ...
                    let native_api_url = native_api_url.clone();
...
fn ollama_native_api_url(api_url: &str) -> String {
    api_url.strip_suffix("/v1").unwrap_or(api_url).to_string()
}

fn skip_auth_for_local_ollama(api_url: &str) -> bool {
    let native_api_url = ollama_native_api_url(api_url);
    native_api_url.starts_with("http://localhost:")
        || native_api_url.starts_with("http://127.0.0.1:")
        || native_api_url == "http://localhost"
        || native_api_url == "http://127.0.0.1"
}
    ...
    fn provided_models(&self, cx: &App) -> Vec<Arc<dyn LanguageModel>> {
        ...
        for model in self.state.read(cx).fetched_models.iter() {
            ...
            if model.display_name.is_none() {
                model.display_name = Some(format!("{} (local)", model.name));
            } else if let Some(display_name) = &model.display_name
                && !display_name.ends_with(" (local)")
            {
                model.display_name = Some(format!("{display_name} (local)"));
            }
