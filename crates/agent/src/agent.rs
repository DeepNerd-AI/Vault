...
impl LanguageModels {
    ...
    fn refresh_list(&mut self, cx: &App) {
        // Prefer authenticated providers, but if none are authenticated
        // include visible providers so the UI can present options and the
        // user can select/authenticate them. This avoids empty model lists
        // in environments where background auth hasn't completed yet.
...
impl NativeAgent {
    ...
    fn build_available_commands_for_project(
        ...
        let mcp_commands: Vec<acp::AvailableCommand> = registry
            .prompts()
    ...
    fn send_init_prompt(
        ...
        cx.spawn(async move |this, cx| {
                ...
                if let Ok(content_str) = serde_json::to_string_pretty(&settings_content) {
                    if std::fs::write(&settings_path, content_str).is_ok() {
                        init_message.push_str(&format!(
                            "- Created **.vault/settings.json** at `{}`\n",
                            root_path.display()
                        ));
                        initialized_count += 1;
                    }
            ...
            if initialized_count == 0 {
                init_message.push_str("- Default fallback configurations initialized inside the memory cache.\n");
            }

            init_message.push_str("- **LSP Support Enabled**: Hover Popovers & Diagnostics are fully configured globally for all languages!\n");
            init_message.push_str("- **Realtime Context Usage**: The DeepAgent panel is tracking token usage and limits in real-time.\n");
            init_message.push_str("- **Brand Assets Overlay**: Customized Vault iconography, theme assets, and premium UX layers successfully deployed.\n\n");
            init_message.push_str("DeepAgent is ready to assist you. Ask any coding question or instruct me to build code!");

            let block = acp::ContentBlock::Text(acp::TextContent::new(init_message));
            acp_thread.update(cx, |acp_thread, cx| {
...
pub static ZED_AGENT_ID: LazyLock<AgentId> = LazyLock::new(|| AgentId::new("DeepAgent"));
