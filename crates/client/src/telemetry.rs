...
impl Telemetry {
    ...
    pub async fn flush_events_inner(self: &Arc<Self>) -> Result<()> {
        // Telemetry is disabled for Vault. Events are not sent to any remote endpoint.
        // Events that were queued are discarded silently.
        let mut state = self.state.lock();
        state.first_event_date_time = None;
        let events = mem::take(&mut state.events_queue);
        state.flush_events_task.take();

        // Optionally log events locally for debugging (disabled by default)
        if let Some(file) = &mut state.log_file {
            let mut json_bytes = Vec::new();
            for event in &events {
                json_bytes.clear();
                if let Ok(_) = serde_json::to_writer(&mut json_bytes, event) {
